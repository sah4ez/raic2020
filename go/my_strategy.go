...
func (s *MyStrategy) DoAttack(e Entity, et Entity, pos Vec2Int32, a Action) {
	...
	aaa := NewAutoAttack(20, []EntityType{EntityTypeBuilderUnit, EntityTypeRangedUnit, EntityTypeMeleeUnit, EntityTypeRangedBase, EntityTypeHouse, EntityTypeMeleeBase, EntityTypeTurret, EntityTypeRangedBase, EntityTypeBuilderBase})
	aa := NewAttackAction(nil, &aaa)
	// aa := NewAttackAction(&e.Id, nil)
	ea := EntityAction{
		MoveAction:   &ma,
		AttackAction: &aa,
	}
	...
	for i, e := range s.defendPoint {
		...
		if d < 30 {
			...
			for i, ss := range s.squards {
				...
				s.PrintVector(centerSS, pos, blue)
			...
			for _, e := range s.squards[s.defendSquard] {
				s.DoAttack(e, t, center, a)
			}
		}
		s.defended[i] = false
	}
...
func (s *MyStrategy) DoAction(a Action) {
	...
	for i, ss := range s.squards {
		...
		if ld > 45 {
			pos, t, d := s.FindEnimyBase(center)
